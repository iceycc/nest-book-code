...
export class Test1597881364764 implements MigrationInterface {
    name = 'Test1597881364764'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("ALTER TABLE `access` ADD `status` tinyint NULL COMMENT '状态,1表示正常,0表示禁用' DEFAULT 1");
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `access` CHANGE `sort` `sort` int NOT NULL COMMENT '排序' DEFAULT '1'");
        await queryRunner.query("ALTER TABLE `access` CHANGE `module_id` `module_id` int NOT NULL COMMENT '父模块id' DEFAULT '-1'");
        await queryRunner.query("ALTER TABLE `access` DROP COLUMN `status`");
    }
